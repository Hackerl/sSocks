...
zero::async::coroutine::Task<Target, std::error_code> readTarget(asyncio::IBufReader &reader) {
    std::byte type[1];
    CO_EXPECT(co_await reader.readExactly(type));
    ...
    case 0: {
        auto hostname = co_await reader.readLine();
        CO_EXPECT(hostname);
